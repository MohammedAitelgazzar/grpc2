...
public class CompteServiceImpl extends CompteServiceGrpc.CompteServiceImplBase {
    ...
    public void compteByType(GetCompteByTypeRequest request, 
        ...
        List<Compte> comptesByType = compteDB.values()
                .stream()
                .filter(compte -> compte.getType() == request.getType())
                .collect(Collectors.toList());

        GetCompteByTypeResponse response = GetCompteByTypeResponse.newBuilder()
    ...
    public void deleteCompte(DeleteCompteRequest request, 
        ...
        Compte removedCompte = compteDB.remove(id);

        DeleteCompteResponse.Builder responseBuilder = DeleteCompteResponse.newBuilder();

        if (removedCompte != null) {
            responseBuilder
                .setSuccess(true)
